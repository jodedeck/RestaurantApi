...
namespace RestaurantApi.Controllers
{
    public class FavoritesController : ApiController
    {
        ...
        // GET: api/GetFavoriteByUserId/1
        [ResponseType(typeof(Favorite))]
        [Route("api/GetFavoriteByUserId/{userId}")]
        public IHttpActionResult GetFavoriteByUserId(int userId)
        {
            var favorite = db.Favorites.Where(f => f.UserId == userId).ToList();
            if (favorite == null)
            {
                return NotFound();
            }
